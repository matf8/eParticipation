...
public class LoginServletToken extends HttpServlet {
	...
	@Inject
	LoginServletToken(AuthConfig config) {
		this.config = config;
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			...
			if (usuario == null) {				// Si retorna null es funcionario
				...
				else urlRedirect = getUriFuncionario(funcionario, idToken);
			} else urlRedirect = getUriCiudadano(usuario, idToken);			
	...
	public String requestUserId(String accessToken) {
		...
		try {		
			...
			//String correo = object.getString("email"); gub uy no da el email xd
			if (ci != null) {
	...
	public String getUriCiudadano(DtCiudadano user, String tokenId) throws Exception {	
		return "https://we-participation.web.elasticloud.uy/loginGubUy?user=" + user.getCedula() 				
				+ "&tokenSesion=" + TokenSecurity.crearToken(user.getCorreo(), user.getRol())
				+ "&tokenID=" + tokenId;
	}
	
	public String getUriFuncionario(DtFuncionario user, String tokenId) throws Exception {	
		return "https://we-participation.web.elasticloud.uy/loginGubUy?user=" + user.getCorreo()
				+ "&tokenSesion=" + TokenSecurity.crearToken(user.getCorreo(), user.getRol())
				+ "&tokenID=" + tokenId;
	}
